...
# 3. weather data 일단위 평균값 계산
def weather_data_day_mean():
    root = get_project_root()
    ...
    wdf_filter = wdf[['temperature', 'uws_10m', 'vws_10m', 'ghi', 'precipitation', 'relative_humidity_1p5m', 'specific_humidity_1p5m',
                      'id_hh', 'id_hs', 'year', 'month', 'day']]

    print('가구 ID 별로 weather data 일평균 계산 시작')
        ...
        for y in u_year:
            ...
            for m in u_month:
                ...
                for d in u_day:
                    ...
                    temp = np.mean(day_filter3.temperature)
    ...
    print('가구 ID 별로 weather data 일평균 계산 완료')
    print('weather data by day가 return 값으로 출력됩니다.')

    return weather_result

# 4.'{user_name}_dataset_day_v2.xlsx' 파일 하나씩 불러서 weather data와 merge
def dataset_weather_merge():
    ...
    # 파일 2 : weather data
    df2 = weather_data_day_mean()

    for i in range(len(user_name)):
        print(f'{user_name[i]} dataset과 weather data와의 merge 시작')

        # 파일 1 : 스마트미터링 자료_시간 단위
        xlsx_name = folder_root + '\\' + f'{user_name[i]}_dataset_day_v2.xlsx'
        df1 = pd.read_excel(xlsx_name, sheet_name='day')

        # merge 실행
        result = pd.merge(df1, df2, how='left', on=['id_hh', 'id_hs', 'year', 'month', 'day'])
...
def final_merge():
    ...
    final_csv_name = df_root + '/' + 'day_merge_data2.csv'

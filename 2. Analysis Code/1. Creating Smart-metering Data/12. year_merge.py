### Code : 스마트미터링 자료 연도별 생성(추가 작업) / weather data + 가구에너지패널조사 data merge
...
def weather_data_month_mean():
    ...
    for i in range(len(user_name)):
        ...
        for y in u_year:
            ...
            temp = np.mean(day_filter1.temperature)
            uws = np.mean(day_filter1.uws_10m)
            vws = np.mean(day_filter1.vws_10m)
            ghi = np.mean(day_filter1.ghi)
            prec = np.mean(day_filter1.precipitation)
            rel_humid = np.mean(day_filter1.relative_humidity_1p5m)
            spec_humid = np.mean(day_filter1.specific_humidity_1p5m)
...
# 4.'{user_name}_dataset_year.xlsx' 파일 하나씩 불러서 weather data와 merge
def dataset_weather_merge():
    ...
    for i in range(len(user_name)):
        ...
        xlsx_name = folder_root + '\\' + f'{user_name[i]}_dataset_year.xlsx'
        df1 = pd.read_excel(xlsx_name, sheet_name='year')

        # merge 실행
        result = pd.merge(df1, df2, how='left', on=['id_hh', 'id_hs', 'year'])
...
def final_merge():
    ...
    final_csv_name = df_root + '/' + 'year_merge_data.csv'

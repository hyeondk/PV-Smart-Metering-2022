### Code : 스마트미터링 자료 시, 일, 월 단위 생성 - 월 / weather data + 가구에너지패널조사 data merge version2
...
def weather_data_month_mean():
    ...
    print('가구 ID 별로 weather data 월평균 계산 시작')
    for i in range(len(user_name)):
        ...
        for y in u_year:
            date_cond1 = (wdf_final.year == y)
            day_filter1 = wdf_final[date_cond1]
            ...
            for m in u_month:
                ...
                temp = np.mean(day_filter2.temperature)
                uws = np.mean(day_filter2.uws_10m)
                vws = np.mean(day_filter2.vws_10m)
                ghi = np.mean(day_filter2.ghi)
                prec = np.mean(day_filter2.precipitation)
                rel_humid = np.mean(day_filter2.relative_humidity_1p5m)
                spec_humid = np.mean(day_filter2.specific_humidity_1p5m)
                ...
                weather_time['temperature'].append(temp)
    ...
    print('가구 ID 별로 weather data 월평균 계산 완료')
    print('weather data by month가 return 값으로 출력됩니다.')

    return weather_result

# 4.'{user_name}_dataset_month_v2.xlsx' 파일 하나씩 불러서 weather data와 merge
def dataset_weather_merge():
    ...
    for i in range(len(user_name)):
        ...
        xlsx_name = folder_root + '\\' + f'{user_name[i]}_dataset_month_v2.xlsx'
        df1 = pd.read_excel(xlsx_name, sheet_name='month')

        # merge 실행
        result = pd.merge(df1, df2, how='left', on=['id_hh', 'id_hs', 'year', 'month'])
...
def final_merge():
    ...
    final_csv_name = df_root + '/' + 'month_merge_data2.csv'

### Code : 스마트미터링 자료 가구별 1년치 데이터 생성(추가 작업)
### Writer : Donghyeon Kim
### Date : 2022.07.05

## 방법론
# 각 가구별로 1년치 데이터 생성.
# 1년 : 2021/5/1 ~ 2022/4/30
# 위 기간에 해당되지 않는 사용자는 고려하지 않음.
...
def get_value_on_use_df():
    ...
    for i in range(len(user_name)):
        ...
        for j in range(len(dir_list)):
            ...
            # [First Data] '2021-05-01' 파일에 대해서만 변수 값 도출
            cond1 = (final_rawdata.year == 2021) & (final_rawdata.month == 5) & (final_rawdata.day == 1)
            if any(cond1) == False:
                continue
            else:
                day_filter1 = final_rawdata[cond1]
            ...
            # [Last Data] '2021-05-01' 파일에 대해서만 변수 값 도출
            cond2 = (final_rawdata.year == 2022) & (final_rawdata.month == 4) & (final_rawdata.day == 30)
            if any(cond2) == False:
                continue
            else:
                day_filter2 = final_rawdata[cond2]
                ...
                # 4) 전력 소비량(total) : consumption
                after_t = day_filter2['전력 소비량(kWh)'].max()
        ...
        xlsx_name = df_root + '/' + f'{user_name[i]}_dataset_housing_year.xlsx'

### Code : 태양광 사용 가구 - Variable Interpolation
...
from pack_utils import get_project_root, get_name_root, get_name_root_use

# 2. 각 사용자 모든 파일 Concatenation
def user_concat(user):
    root = get_project_root()
    df_result = pd.DataFrame()

    print(f'{user} 태양광 사용 가구 Dataset : Concatenation & Interpolation 시작')
    ...
    df_result['에너지 수율(kWh)'] = df_result['에너지 수율(kWh)'].fillna(df_result['에너지 수율(kWh)'].interpolate())

    # 결과물 저장
    df_root = os.path.join(root, 'data_concat_use')
    ...
    print(f'{user} 태양광 사용 가구 Dataset : Concatenation & Interpolation 종료')
    return

# 3. Interpolated Data - 1시간 단위 Dataset 생성
def make_dataset_hour(user):
    root = get_project_root()

    print(f'{user} 태양광 사용 가구 Dataset : 1시간 단위 변경 시작')
    ...
    data_time['에너지 수율(kWh)'] = [] # 태양광 발전량(생산량)
    data_time['전력 소비량(kWh)'] = [] # 전력 소비량
    update = 0

    folder_root = os.path.join(root, 'data_concat_use')
    ...
    for row in rawdata.itertuples():
        if update and row.minute == 59:
            ...
            yield_ = after_y - before_y # 시간당 에너지 수율 -> 태양광 발전량(생산량)
            export_ = after_e - before_e # 시간당 수출 된 에너지 -> 잉여 전력량
            time_total = consum_ + yield_ - export_ # 시간당 전력 소비량
    ...
    df_root = os.path.join(root, 'data_hour_f')
    if not os.path.isdir(df_root):
        os.makedirs(df_root)

    xlsx_name = df_root + '/' + f'{user}_hour.xlsx'
    data_frame_time.to_excel(xlsx_name, sheet_name='hour', index=False)
    print(f'{user} 태양광 사용 가구 Dataset : 1시간 단위 변경 종료')
    return

# 4. Interpolated Data - 1일 단위 Dataset 생성
def make_dataset_day(user):
    root = get_project_root()

    print(f'{user} 태양광 사용 가구 Dataset : 1일 단위 변경 시작')
    ...
    for y in u_year:
        ...
        for m in u_month:
            ...
            for d in u_day:
                ...
                yield_ = np.sum(day_filter3['에너지 수율(kWh)']) # 일일 에너지 수율 -> 태양광 발전량(생산량)

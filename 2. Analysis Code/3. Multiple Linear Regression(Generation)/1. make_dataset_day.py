### Code : Interpolated Data - 일 단위 data 생성
### Writer : Donghyeon Kim
### Date : 2022.08.14

## 방법론
# 일별로 max - min 계산 진행 -> 그리드 소비, 수출된 에너지, 에너지 수율 모두 적용.
...
def get_value_on_use_df():
    ...
    for i in range(len(user_name)):
        ...
        df_root = os.path.join(root, 'data_revised_day')
        if not os.path.isdir(df_root):
            os.makedirs(df_root)

        xlsx_name = df_root + '/' + f'{user_name[i]}_dataset_revised_day.xlsx'

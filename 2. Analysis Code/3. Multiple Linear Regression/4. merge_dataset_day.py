...
def weather_user_merge(user):
    # 사용자 데이터 호출
    root = get_project_root()
    user_folder_root = os.path.join(root, 'data_revised_day')
    xlsx_name = user_folder_root + '\\' + f'{user}_dataset_revised_day.xlsx'
        ...
        for m in u_month:
            ...
            for d in u_day:
                ...
                if m in [1, 2, 11, 12]:
                    ...
                    for h in cond_hour:
                        temp1 = day_filter3.loc[day_filter3.hour == h, :]
                    ...
                    df_weather_f['day'].append(d)
                elif m == 3:
                    cond_hour = range(8, 19+1) # 일사량 : 8 ~ 19시

                    for h in cond_hour:
                        temp2 = day_filter3.loc[day_filter3.hour == h, :]
                    ...
                    for h in cond_hour:
                        temp3 = day_filter3.loc[day_filter3.hour == h, :]
                    ...
                    for h in cond_hour:
                        temp4 = day_filter3.loc[day_filter3.hour == h, :]
    ...
    # Data Merge
    df_weather_use = pd.merge(df_weather_f, df_user, how='left', on=['year', 'month', 'day'])
...
def weather_user_visibility_merge(user):
    ...
    # 두 데이터 Merge
    reg_data = pd.merge(df_weather_use, df_asos_f, how='left', on=['year', 'month', 'day', 'place'])

    # Merged Data를 DF로 설정
    reg_data = pd.DataFrame(reg_data)

    result_root = os.path.join(root, 'data_merge_day')
    if not os.path.isdir(result_root):
        os.makedirs(result_root)

    xlsx_name = result_root + '/' + f'{user}_dataset_merge_day.xlsx'
